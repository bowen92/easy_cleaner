...
class QualityClassifer(object):
    ...
    def classify(self, text):
        ...
        if self.language == 'cn':
            ...
            segs = list(filter(lambda x:((len(x)>1) and (x not in cn_stopwords)), segs))
            text = " ".join(segs)
        if len(text)>0:
